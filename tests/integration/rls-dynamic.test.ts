...
/**
 * Activate the dynamic-RLS flag for the current transaction only.
 * Mirrors what `pnpm db:rls on` would do at the database level, but
 * scoped to the test's transaction so other tests don't see it.
 */
async function withFlagOn<T>(
  tx: Parameters<Parameters<typeof runAs>[2]>[0],
  fn: () => Promise<T>,
): Promise<T> {
  await tx.execute(sql`SET LOCAL blacknel.rls_dynamic = 'on'`);
  return fn();
}
...
describe('Phase 11 / C42c — RLS dynamic policies privilege escalation matrix', () => {
  ...
  describe('rollback — flag flip on→off restores tenant-only behavior', () => {
    it('viewer UPDATE: 0 rows under flag=on, then 1 row after SET LOCAL = off', async () => {
      // First tx: flag on → 0 rows.
      ...
      // Second tx: explicit off → restrictive policy short-circuits, UPDATE works.
      const allowed = await runAs<Array<{ id: string }>>(
        fixture.db,
        { orgId: ORG, userId: VIEWER, role: 'viewer' },
        async (tx) => {
          await tx.execute(sql`SET LOCAL blacknel.rls_dynamic = 'off'`);
          return tx
            .update(posts)
