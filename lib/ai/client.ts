 ...
 *
 *   real  ⟺  env.BLACKNEL_USE_REAL_AI === true
 *        AND env.ANTHROPIC_API_KEY is set
 *        AND app_settings.use_real_ai === 'on'
 *        → the orchestrator (dedup/rate-limit/budget/redact → Anthropic primary
 *          with OpenAI fallback → persist) — lib/ai/orchestrator.ts.
 *   else  →  deterministic mock (adapter-mock), uncapped + content-storing.
 *
 ...
 */
const realClient: AiClient = { generate: generateReal };
...
export async function resolveAiAdapter(): Promise<AiClient> {
  ...
  return (await isRealAiEnabled()) ? realClient : adapterMock;
}
