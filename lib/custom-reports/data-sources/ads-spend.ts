      ...
      await ctx.tx.execute(
        sql`SELECT COALESCE(SUM(spend_usd_cents), 0)::bigint AS cents
             ...
             WHERE organization_id = ${ctx.orgId}
               AND day >= ${ctx.rangeStart.toISOString().slice(0, 10)}::date
               AND day <= ${ctx.rangeEnd.toISOString().slice(0, 10)}::date`,
      ),
    );
...
async function loadTimeseries(
  ...
  if (metric === 'spend_usd') {
    const rows = normalizeRows<{ day: string; v: number | null }>(
      await ctx.tx.execute(
        sql`SELECT to_char(day, 'YYYY-MM-DD') AS day,
